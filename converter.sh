...
# ============================================================
# Read MortazDev animation config (STEP2)
# ============================================================
ANIMATION_SELECTION="false"  # default = animation enabled
MORTAZ_CONFIG="./mortazdev_config.json"
if [[ -f "$MORTAZ_CONFIG" ]]; then
  ANIMATION_SELECTION=$(jq -r '
    .Animation_Selection.animation.selection // false
  ' "$MORTAZ_CONFIG" 2>/dev/null)
  status_message info "MortazDev animation selection = ${ANIMATION_SELECTION}"
else
  status_message info "No mortazdev_config.json found → animation enabled by default"
fi
...
# 🧱 Skip unwanted asset folders (betterhud, nameplates, modelengine) MortazDev Edit
...
#step4
status_message process "Generating union atlas arrays"
...
# ── Zicon: batch render 3D → 64×64 PNG via Three.js + headless-gl ───────────
status_message process "Generating zicon batch (Three.js + headless-gl)"
mkdir -p ./target/rp/textures/zicon
rm -f scratch_files/zicon_jobs.csv scratch_files/zicons.csv

zicon_skip=0

while IFS=, read -r file gid generated namespace model_path model_name path_hash geometry
do
  [[ "$namespace" == "modelengine" ]] && { (( zicon_skip++ )); continue; }
  [[ ! -f "$file" ]]                  && { (( zicon_skip++ )); continue; }

  # generated=true (builtin/generated, no elements) → copy texture directly
  if [[ "$generated" == "true" ]]; then
    if [[ -n "$model_path" ]]; then
      out_icon="./target/rp/textures/zicon/${namespace}/${model_path}/${model_name}.png"
      icon_ref="textures/zicon/${namespace}/${model_path}/${model_name}"
    else
      out_icon="./target/rp/textures/zicon/${namespace}/${model_name}.png"
      icon_ref="textures/zicon/${namespace}/${model_name}"
    fi
    mkdir -p "$(dirname "$out_icon")"
    if [[ ! -f "$out_icon" ]]; then
      tex_ref=$(jq -r '.textures | to_entries | map(select(.key != "particle")) | .[0].value // empty' "$file" 2>/dev/null)
      if [[ -n "$tex_ref" ]]; then
        if [[ "$tex_ref" == *:* ]]; then tns="${tex_ref%%:*}"; tid="${tex_ref#*:}";
        else tns=$(echo "$file" | sed 's|.*assets/||; s|/.*||'); tid="$tex_ref"; fi
        for p in "./assets/$tns/textures/$tid.png" \
                 "./assets/$tns/textures/item/$tid.png" \
                 "./assets/$tns/textures/block/$tid.png" \
                 "./assets/$tns/textures/items/$tid.png"; do
          if [[ -f "$p" ]]; then
            w=$(identify -format "%w" "$p" 2>/dev/null || echo 0)
            if [[ "$w" -eq 64 ]]; then cp "$p" "$out_icon"
            else convert "$p" -filter Point -resize 64x64! PNG32:"$out_icon" 2>/dev/null; fi
            break
          fi
        done
      fi
    fi
    [[ -f "$out_icon" ]] && echo "${path_hash},${icon_ref}" >> scratch_files/zicons.csv
    (( zicon_skip++ ))
    continue
  fi
  ...
  # Cache hit: icon already rendered, just add to CSV
  if [[ -f "$out_icon" ]]; then
    echo "${path_hash},${icon_ref}" >> scratch_files/zicons.csv
    continue
  fi

  echo "${file},${out_icon},${path_hash},${icon_ref}" >> scratch_files/zicon_jobs.csv

done < <(jq -r \
  '.[] | [.path,.geyserID,.generated,.namespace,.model_path,.model_name,.path_hash,.geometry]
       | @tsv | gsub("\t";",")' config.json)

# ── Run node render_icon.js ONCE for all pending jobs ────────────────────────
if [[ -s scratch_files/zicon_jobs.csv ]]; then
  job_count=$(wc -l < scratch_files/zicon_jobs.csv)
  status_message process "Rendering ${job_count} zicons (Three.js + headless-gl, 1 node process)..."

  # render_icon.js expects: model_path,output_png
  awk -F',' '{print $1","$2}' scratch_files/zicon_jobs.csv > scratch_files/render_jobs.csv

  node render_icon.js scratch_files/render_jobs.csv ./assets 2>&1 | \
    while IFS= read -r line; do
      [[ "$line" == ok:* ]] && status_message completion "${line}" || status_message error "${line}"
    done

  # Build zicons.csv; ImageMagick fallback for any icon that failed to render
  while IFS=, read -r file out_icon path_hash icon_ref; do
    if [[ -f "$out_icon" ]]; then
      echo "${path_hash},${icon_ref}" >> scratch_files/zicons.csv
    else
      # Fallback: copy texture PNG directly (for flat/billboard/skill models)
      tex_ref=$(jq -r '.textures | to_entries | map(select(.key != "particle")) | .[0].value // empty' "$file" 2>/dev/null)
      if [[ -n "$tex_ref" ]]; then
        if [[ "$tex_ref" == *:* ]]; then tns="${tex_ref%%:*}"; tid="${tex_ref#*:}";
        else tns=$(echo "$file" | sed 's|.*assets/||; s|/.*||'); tid="$tex_ref"; fi
        for p in "./assets/$tns/textures/$tid.png" \
                 ...
                 "./assets/$tns/textures/items/$tid.png"; do
          if [[ -f "$p" ]]; then
            # Copy texture directly — preserve original pixel art, just ensure 64x64
            w=$(identify -format "%w" "$p" 2>/dev/null || echo 0)
            if [[ "$w" -eq 64 ]]; then
              cp "$p" "$out_icon"
            else
              convert "$p" -filter Point -resize 64x64! PNG32:"$out_icon" 2>/dev/null
            fi
            [[ -f "$out_icon" ]] && echo "${path_hash},${icon_ref}" >> scratch_files/zicons.csv
            break
          fi
      ...
      [[ ! -f "$out_icon" ]] && status_message error "zicon fallback failed (no texture): ${out_icon}"
    fi
  done < scratch_files/zicon_jobs.csv
fi

rendered_ok=$(wc -l < scratch_files/zicons.csv 2>/dev/null || echo 0)
status_message completion "zicon done — rendered/cached:${rendered_ok} skip:${zicon_skip}\n"

# ── Merge icons.csv (2D flat) + zicons.csv → item_texture.json ───────────────
{
  [[ -f scratch_files/icons.csv  ]] && cat scratch_files/icons.csv
  [[ -f scratch_files/zicons.csv ]] && cat scratch_files/zicons.csv
} | sort -u \
  | jq -cR 'split(",")' \
  | jq -s 'map({(.[0]): {"textures": (.[1] | gsub("//";"/"))}}) | add // {}' \
  > scratch_files/all_icons.json

jq -s '.[0] as $icons | .[1] | .texture_data += $icons' \
  scratch_files/all_icons.json ./target/rp/textures/item_texture.json \
  | sponge ./target/rp/textures/item_texture.json

status_message completion "item_texture.json updated (2D flat + zicon 3D renders)"
